...
def find_bilingual_table(doc: Document) -> Tuple:
    """
...
def analyze_document(docx_path: str, char_limit: int) -> List[Dict]:
    """
    ...
    for row in table.rows[header_row_index + 1:]:
        ...
        # Keep all lines, including empty; we only ignore lines that are
        # completely empty when computing lengths.
...
def build_html_report(
    ...
    rows_html: List[str] = []
    for v in violations:
        # Only show the numeric segment ID part (everything before first space)
        full_id = v["id"]
        ...
        # For reset button – store the original highlighted HTML as attribute
    ...
    # HTML template – layout & colors like your stable 3.22, plus small additions
...
def main():
    ...
    # Auto-detect file in current folder (unchanged behaviour)
    current_dir = os.getcwd()
    files = os.listdir(current_dir)

    docx_files = [f for f in files if f.lower().endswith(".docx")]
    rtf_like_files = [
        f for f in files if f.lower().endswith(".rtf") or f.lower().endswith(".rtx")
    ]
    ...
    if ext != ".docx":
        print(
            "Error: this version works with DOCX only. "
            "Please export a bilingual DOCX from memoQ."
        )
        return
    ...
    violations = analyze_document(input_path, char_limit)
